...
must be no longer than eight pages in length.
...
\end{center}
\caption{
   method 1:未采用预训练生成器模型和随机退化的IRSRGAN。
method 2:未采用与训练生成器模型但采用了随机退化的IRSRGAN。
method 3:采用了预训练生成器但未采用随机退化的IRSRGAN。
method 4:采用了预训练模型和随机退化的IRSRGAN。 }\label{tab:table}
\end{table}
...
本文在ESRGAN的基础上引入了全局特征网络和随机退化下采样，显著的提高了单帧红外图像超分辨率重建的效果。
未来红外图像超分辨率重建仍然有很大的提升空间。
